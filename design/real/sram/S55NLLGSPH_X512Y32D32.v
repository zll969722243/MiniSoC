/*
    Copyright (c) 2019 SMIC
    Filename:      S55NLLGSPH_X512Y32D32.v
    IP code :      S55NLLGSPH
    Version:       1.1.a
    ...
    Configuration: -instname S55NLLGSPH_X512Y32D32 -rows 512 -bits 32 -mux 32 
    Redundancy: Off
    Bit-Write: On
*/
...
module S55NLLGSPH_X512Y32D32(
 ...
 reg 			BWEN31_flag;
  ...
  always @(CLK_int,CLK)
    begin
	  ...
	  for(i=0;i<Word_Depth;i=i+1)
    	    mem_array[i]={Bits{1'bx}};$display("2==========================ram init");
    	  Q_latched={Bits{1'bx}};
          rw_mem;
          //$display("2==========================addr:%h-data:%h",A,D);
          end
      endcase
    LAST_CLK=CLK_int;
    //$display("3==========================addr:%h-data:%h",A,D);
   end
  ...
  always @(CEN_flag
    ...
    begin
      ...
      rw_mem;
	    //$display("4==========================addr:%h-data:%h",A_latched,mem_array[0]);
   end
                  ...
                  begin
                    if (^(A_latched)==1'bx)
                      begin
                        for (i=0; i<Word_Depth; i=i+1)
                          begin
                            ...
                            mem_array[i]=data_tmp;$display("3==========================ram init");
                          end
                        for (i=lb; i<=hb; i=i+1) Q_latched[i]=1'bx;
                      end
                    else
                      begin
                        ...
                        mem_array[A_latched]=data_tmp;$display("4==========================ram init");
                        for (i=lb; i<=hb; i=i+1) Q_latched[i]=data_tmp[i];
                      end
                  end
                else
                      ...
                      begin
                        for (i=0; i<Word_Depth; i=i+1)
                          begin
                            data_tmp=mem_array[i];
                            for (j=lb; j<=hb; j=j+1) data_tmp[j]=1'bx;
                            mem_array[i]=data_tmp;$display("5==========================ram init");
                          end
                      end
                    else
                      begin
                        ...
                        mem_array[A_latched]=data_tmp;$display("6==========================ram init");
                      end
                 end
               end
				//$display("==========================addr:%h-data:%h",A_latched,mem_array[0]);
             end
           else
             begin
               for (wenn=0; wenn<Wen_Width; wenn=wenn+1)
                      ...
                      begin
                        for (i=0; i<Word_Depth; i=i+1)
                          begin
                            data_tmp=mem_array[i];
                            for (j=lb; j<=hb; j=j+1) data_tmp[j]=1'bx;
                            mem_array[i]=data_tmp;$display("8-1==========================ram init");
                          end
                      end
                    else
                      begin
                        data_tmp=mem_array[A_latched];
                        for (i=lb; i<=hb; i=i+1) data_tmp[i]=1'bx;
                        mem_array[A_latched]=data_tmp;$display("8==========================ram init");
                      end
                 end
               end
             end
                    ...
                    begin
                      for (i=0;i<Word_Depth;i=i+1)
                        begin
                          data_tmp=mem_array[i];
                          for (j=lb;j<=hb;j=j+1) data_tmp[j]=1'bx;
                          mem_array[i]=data_tmp;$display("9==========================ram init");
                        end
                    end
                  else
                    begin
                      data_tmp=mem_array[A_latched];
                      for (i=lb;i<=hb;i=i+1) data_tmp[i]=1'bx;
                      mem_array[A_latched]=data_tmp;$display("A==========================ram init");
                    end
                end
            end
        end
  ...
  endtask
   ...
   begin
     for(i=0;i<Word_Depth;i=i+1)
     mem_array[i]={Bits{1'bx}};$display("B==========================ram init");
   end
   endtask
 ...
 BWEN_flag = {
  ...
  specify
    ...
    $setuphold(posedge CLK &&& CE_flag,posedge A[12],0.500,0.250,A12_flag);
    $setuphold(posedge CLK &&& CE_flag,negedge A[12],0.500,0.250,A12_flag);
    $setuphold(posedge CLK &&& CE_flag,posedge A[13],0.500,0.250,A13_flag);
    $setuphold(posedge CLK &&& CE_flag,negedge A[13],0.500,0.250,A13_flag);
    ...
    $period(posedge CLK,2.963,CLK_CYC_flag);
    $width(posedge CLK,0.889,0,CLK_H_flag);
    $width(negedge CLK,0.889,0,CLK_L_flag);
